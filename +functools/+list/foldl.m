...
%
%   foldl(f, s, [x1, x2, ..., xn] => f( ... f(f(f(s, x1),x2), ...), xn)
%
...
%       inputs (\acc head_list) and returns one output. It traverses the
%       list from left to right.
%
%   acc_with_init = foldl(f, acc) returns a function that takes
%       in a list. It performs the same as above, but with a constant
%       initial acc value and constant f.
%
...
%
%   See also FOLDR, FOLDL1, FOLDR1, BUFFERFOLDL, BUFFERFOLDR
